...
def _make_grouped_bar(devices, mlr_vals, rf_vals, y_title, mlr_hover, rf_hover, is_dark=False):
    ...
    fig.update_layout(
        ...
        margin=dict(t=45, b=140, l=40, r=20),
...
def _make_intensity_chart(is_dark=False):
    ...
    fig.update_layout(
        ...
        font=dict(color='#888'),
        legend=dict(orientation="h", yanchor="top", y=-0.5, xanchor="center", x=0.5),
        margin=dict(t=30, b=140, l=40, r=20),
        yaxis=dict(gridcolor='rgba(150,150,150,0.1)'),
        hovermode="x unified"
    )
    return fig
...
def create_ml_layout(is_dark=False) -> html.Div:
    ...
    fig_mae = _make_grouped_bar(devices, mlr_mae, rf_mae, "Mean Absolute Error (METs)",
                                "<b>%{x}</b><br>Ridge Regression MAE: %{y:.3f} METs<extra></extra>",
    ...
    fig_rmse = _make_grouped_bar(devices, mlr_rmse, rf_rmse, "Root Mean Squared Error (METs)",
                                 "<b>%{x}</b><br>Ridge Regression RMSE: %{y:.3f} METs<extra></extra>",
    ...
    fig_r2 = _make_grouped_bar(devices, mlr_r2, rf_r2, "R-squared (R²)",
                               "<b>%{x}</b><br>Ridge Regression R²: %{y:.3f}<extra></extra>",
